...
def search_by_date() -> str:
    ...
    hide_old_infants = bool(request.args.get('hide_old_infants'))
    mode = request.args.get('mode', 'all')
    error = ""
    html = DEFAULT_HTML
    try:
        html = generate_html_by_date(
            datestr, lookback_days=num_days, hide_old_infants=hide_old_infants,
            mode=mode
        )
    except FileNotFoundError:
        error = f"No cached results found for {date}. Please try a different date."
    return render_template_string(html, today=date, lookback_days=num_days, hide_old_infants=hide_old_infants, error=error, mode=mode)