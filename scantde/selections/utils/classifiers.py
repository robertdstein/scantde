...
def apply_classifier(
    ...
    chunks = [x for x in selection.split("_") if x != "tdescore"]
    selection_parsed = "_".join(chunks)

    selection_prefix = "" if len(selection_parsed) == 0 else f"_{selection_parsed}"

    if classifier == "hostfast":
        classifier_name = f"hostonlyfast{selection_prefix}"
        columns = fast_host_columns
    elif classifier == "host":
        classifier_name = f"hostonly{selection_prefix}"
        columns = host_columns
    elif classifier == "infant":
        classifier_name = f"infant{selection_prefix}"
        columns = infant_columns
    elif classifier == "week":
        classifier_name = f"week{selection_prefix}"
        columns = week_columns
    elif classifier == "month":
        classifier_name = f"month{selection_prefix}"
        columns = month_columns
    elif classifier == "full":
        classifier_name = f"full{selection_prefix}"
        columns = post_peak
    elif "thermal" in classifier:
        window = classifier.split("_")[-1]
        try:
            window = float(window)
            classifier_name = f"thermal{selection_prefix}_{window:.0f}"
        except ValueError:
            window = None
            classifier_name = f"thermal{selection_prefix}_all"
        columns = get_thermal_columns(
            window,
            include_host = selection in ["tdescore"],
            include_offset = selection in ["tdescore", "tdescore_nohostinfo"],
        )
    else:
    ...
    logger.info(f"Applying classifier: tdescore_{classifier_name}")
